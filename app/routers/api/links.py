...
def list_links(slug: str, db: Session = Depends(get_db)) -> list[QuickLink]:
    project = get_project_or_404(slug, db)
    return project.links


@router.post("", response_model=QuickLinkResponse, status_code=status.HTTP_201_CREATED)
def create_link(slug: str, data: QuickLinkCreate, db: Session = Depends(get_db)) -> QuickLink:
    project = get_project_or_404(slug, db)
...
def update_link(slug: str, link_id: int, data: QuickLinkUpdate, db: Session = Depends(get_db)) -> QuickLink:
    project = get_project_or_404(slug, db)
...
def delete_link(slug: str, link_id: int, db: Session = Depends(get_db)) -> None:
    project = get_project_or_404(slug, db)
    link = _get_link_or_404(link_id, project.id, db)
    db.delete(link)
    db.commit()
